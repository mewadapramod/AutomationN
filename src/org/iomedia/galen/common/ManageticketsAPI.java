...
import org.iomedia.galen.common.ManageticketsAAPI.Event;
...
public HashMap<Integer, Event> getEventIdWithAllTktsFlags() throws Exception {
  ...
  public String[] renderTicketDetailsEDP(String ticket, boolean isBarcodeEnabled) throws Exception {
		String barcode = null;
		int venueId = 0;
		String[] ticket_id = ticket.split("\\.");
		int eventId = Integer.valueOf(ticket_id[0].trim());
		String accessToken = getAccessToken(emailaddress, password);
		String seatLocator = null;
		String phone = null;
		...
		getAccountId(accessToken);
		String member_id = Dictionary.get("member_id");
		String accountnumber = member_id.split("\\.")[1];
		JSONObject jsonObject = aapi.getAllEventsForMember(member_id);
		JSONObject embedded = (JSONObject) jsonObject.get("_embedded");
		JSONArray events = embedded.getJSONArray("events");
		if (events != null) {
			for (int i = 0; i < events.length(); i++) {
				int event_Id = events.getJSONObject(i).getInt("id");
				if (eventId == event_Id) {
					venueId = events.getJSONObject(i).getInt("venueId");
					break;
				}
			}
		}
		...
		JSONObject eventJsonObject = aapi.getAllSectionsForAnyEvent(String.valueOf(eventId));
		embedded = (JSONObject) eventJsonObject.get("_embedded");
		JSONArray eventSections = embedded.getJSONArray("sections");
		String sectionID = "" + ((JSONObject) eventSections.get(0)).getInt("id") + "";

		JSONObject sections = aapi.getSectionDetailsForEvent(String.valueOf(eventId), sectionID);
		JSONArray rows = (JSONArray) sections.get("rows");
		for (int k = 0; k < rows.length(); k++) {
			JSONObject ticket_ = (JSONObject) rows.get(k);
			JSONArray tickets = (JSONArray) ticket_.get("seats");
			for (int l = 0; l < tickets.length(); l++) {
				JSONObject seat = (JSONObject) tickets.get(l);
				String ticket_Id = (String) seat.get("id");
				if (ticket_Id.equals(ticket)) {
					if (driverType.trim().toUpperCase().contains("ANDROID"))
						barcode = seat.getString("barcode").trim() + "j";
					else if (driverType.trim().toUpperCase().contains("IOS"))
						barcode = seat.getString("barcode").trim() + "k";
					else
						barcode = seat.getString("barcode").trim() + "d";
					barcode = barcode.trim().equalsIgnoreCase("") ? "-" : barcode;
					
					// To be used for build number 5.1.x and above
					// seatLocator = seat.getString("gatePortalLabel").trim()+" / "+seat.getString("gatePortal").trim();
					seatLocator = "Entry Info " + seat.getString("gatePortal").trim();
					
					String seatNumber = seat.getString("formattedLabel").replace("Seat", "").trim().toUpperCase();
					String rowName = ticket_.getString("formattedLabel").replace("Row", "").trim().toUpperCase();
					String sectionName = sections.getString("formattedLabel").replace("Section", "").trim().toUpperCase();
					
					if(!seatNumber.equals("") && !rowName.equals("") && !sectionName.equals("")) {
						  sectionRowSeat = "SEC "+sectionName+" | ROW "+rowName+" | SEAT "+seatNumber;
					  }
					  else if(seatNumber.equals("") && !rowName.equals("") && !sectionName.equals("")) {
						  sectionRowSeat = "SEC "+sectionName+" | ROW "+rowName;
					  }
					  else if(!seatNumber.equals("") && rowName.equals("") && !sectionName.equals("")) {
						  sectionRowSeat = "SEC "+sectionName+" | SEAT "+seatNumber;
					  }
					  else if(!seatNumber.equals("") && !rowName.equals("") && sectionName.equals("")) {
		...
		JSONObject termsOfUse = aapi.getTermsOfUse(eventId+"");
		String terms_conditions = termsOfUse.getString("text").trim();
		
		JSONObject venueDetails = aapi.getVenue(venueId+"");
		phone = venueDetails.getString("phone");
		
		JSONObject addressJson = (JSONObject) venueDetails.get("address");
		JSONObject streetJson = (JSONObject) addressJson.get("streetAddress");
		
		venueName = venueDetails.getString("name")+streetJson.getString("line1")+addressJson.getString("city")+", "+addressJson.getString("stateCode")+" "+addressJson.getString("postalCode");
		
		if(hasBarcode && isBarcodeEnabled)
			  return new String[]{accountnumber, barcode, sectionRowSeat, seatLocator, venueName, validatePhoneNumber(phone), terms_conditions};
		  else
			  return new String[]{accountnumber, sectionRowSeat, seatLocator, venueName, validatePhoneNumber(phone), terms_conditions}; 
  }
  
  public String[] renderTicketDetails(String ticket, boolean isBarcodeEnabled) throws Exception {
	  String accessToken = getAccessToken(emailaddress, password);
	  boolean hasBarcode = false;
	  getAccountId(accessToken);
	  String member_id = Dictionary.get("member_id");
...
//		  }
	...
	private String validatePhoneNumber(String phoneNo) {
		// handle phone format "1234567890"
		if (phoneNo.matches("\\d{10}"))
			return phoneNo;
		// handle phone format with -, . or spaces
		else if (phoneNo.matches("\\d{3}[-\\.\\s]\\d{3}[-\\.\\s]\\d{4}"))
			return phoneNo.substring(0, 3) + phoneNo.substring(4, 7) + phoneNo.substring(8);
		// handle phone format with area code is in braces ()
		else if (phoneNo.matches("\\(\\d{3}\\)[-\\.\\s]\\d{3}-\\d{4}"))
			return phoneNo.substring(1, 4) + phoneNo.substring(6, 9) + phoneNo.substring(10);
		// return same number if nothing matches the input
		else
			return phoneNo;
	}
