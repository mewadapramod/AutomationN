...
public class CMS extends BaseUtil {
	...
	//NAMMe Locators //a[contains(.,'Emails')]/following-sibling::ul
	private By expandMarketingExperience = By.xpath("//a[contains(.,'Marketing Experience')]");
	private By marketingExperienceAddPage = By.xpath("//p[contains(text(),'Add Page')]");
	private By closebuttonvideo = By.xpath("//span[@class='ui-button-icon ui-icon ui-icon-closethick'] | //button[@title='Close']");
	...
	private By tagVersion = By.id("block-sitebuildnumberblock");
...
//		type(firstInput, "First name", firstNameSignUpTextClaim);
	...
	public boolean checkBarcodeEnabled() {
        try {
            long value =  (long)((JavascriptExecutor)this.getDriver()).executeScript("return window.drupalSettings.componentConfigData.siteconfig.manage_ticket_configuration.barcode_number_enabled");
            if(value==1)
                return true;
        } catch (WebDriverException var5) {
            var5.getMessage();
	...
	public void clickAddpageButtonUnderMarketingExperience() {
		Dictionary.put("tagVersion", getText(tagVersion).split(":")[1].trim());
		JavascriptExecutor jse = (JavascriptExecutor) getDriver();
		String ulClass = (String) jse.executeScript("return arguments[0].getAttribute('class');", getElementWhenPresent(By.xpath(getLocatorValue(expandMarketingExperience)+"/following-sibling::ul")));
		if(ulClass.equalsIgnoreCase("menu collapse")) {
			click(expandMarketingExperience, "Expand Marketing Experience");
		}
//		if(checkElementPresent(expandMarketingExperience, 2))
//			click(expandMarketingExperience, "Expand Marketing Experience");
	...
	public void fillPageSettingsData(String accessLevelInput) throws Exception {
		...
		if (checkElementPresent(pageName, 5)) {
			type(pageName, "Page Name Text Box", "TestAutomation", 2);
		}
		
		if (checkElementPresent(pageTitle, 5)) {
			type(pageTitle, "Page Title Text Box", "TestAutomation", 2);
		}
		System.out.println(getElementWhenPresent(accesslevel("Public")).isSelected());
		System.out.println(getElementWhenPresent(accesslevel("Logged In")).isSelected());
		JavascriptExecutor execute = (JavascriptExecutor) getDriver();
		if (accessLevelInput.contains("Public")) {
			if (!getElementWhenPresent(accesslevel("Public")).isSelected()) {
				execute.executeScript("arguments[0].click();", getElementWhenPresent(accesslevel("Public")));
			}
		} else {
			if (getElementWhenPresent(accesslevel("Public")).isSelected()) {
				execute.executeScript("arguments[0].click();", getElementWhenPresent(accesslevel("Public")));
		...
		if (accessLevelInput.contains("Logged In")) {
			if (!getElementWhenPresent(accesslevel("Logged In")).isSelected()) {
				execute.executeScript("arguments[0].click();", getElementWhenPresent(accesslevel("Logged In")));
			}
		} else {
			if (getElementWhenPresent(accesslevel("Logged In")).isSelected()) {
				execute.executeScript("arguments[0].click();", getElementWhenPresent(accesslevel("Logged In")));
		...
		click(savebuttonPagesetting, "Save Button");
		sync(5000l);
