...
//            }
	...
	public void errorLogin(String emailaddress, String password) throws Exception {
		clickSignInLink(null);
		typeEmailAddress(emailaddress);
		typePassword(password);
		clickRememberMe();
		clickSignInButton();
		String errormessage=getText(errorNotificationMessage, 2);
		Dictionary.put("errorMessage", errormessage);
	}
	
	public void verifyErrorMessage() {
	String expectedErrorMsg = "Error\n" + 
			"Unable to sign in? Make sure your email and password are correct.";
	Assert.assertEquals(Dictionary.get("errorMessage"), expectedErrorMsg, "Error message verified on login popup");
