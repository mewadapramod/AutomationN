...
//		while (base.Dictionary.get("ACTUAL_MSG_COUNT").trim().equalsIgnoreCase("")) {
	...
	public void verify_barcode(String ticketIds, String index, String barcode) throws Exception {
		...
		String[] details = api.renderTicketDetails(ticketId, true);
	...
	@When("^User provide wrong (.+) and (.+)$")
	public void user_provide_wrong(String emailaddress, String password) throws Exception {
		emailaddress=(String) base.getGDValue(emailaddress);
		password=(String) base.getGDValue(password);
	    homepage.errorLogin(emailaddress,password);
	}
	
	@Then("^User verify errormessage when user tries to sign in$")
	public void user_verify_errormessage_when_user_tries_to_sign_in() throws Exception {
