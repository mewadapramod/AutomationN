...
import org.iomedia.galen.pages.NamMe;
...
public class CMSSteps {
	...
	public CMSSteps(SuperAdminPanel adminPanel, BaseUtil base, Utils utils, org.iomedia.framework.Assert Assert, AddNewPage addNewPage,ContentPageEdit contentPageEdit,CMS cms,NamMe namMe)
	{
...
//User configued paypal endpoints based on dsn
   ...
   @Then("^user drag \"([^\"]*)\" from widget section to page$")
   public void user_drag_from_widget_section_to_page(String widgetName) throws Exception {
       namMe.dragAndDropWidget(widgetName);
   }
   
   @Then("^User start editing \"([^\"]*)\"$")
   public void user_start_editing(String widgetName) throws Exception {
       namMe.editWidget(widgetName);
   }
   
   @When("^user clicks on Publish button$")
   public void user_clicks_on_Publish_button() throws Exception {
       namMe.clickPublishButton();
   }

   @Then("^popup appers to edit page in other language$")
   public void popup_appers_to_edit_page_in_other_language() throws Exception {
	   namMe.verifyPopupPresent();
   }

   @Then("^user clicks on continue$")
   public void user_clicks_on_continue() throws Exception {
       namMe.clickContinueButton();
	   
   }

   @Then("^final publish button appears and user clicks publish$")
   public void final_publish_button_appears_and_user_clicks_publish() throws Exception {
	   namMe.clickPublishButton();
   }

   @Then("^user landed on view pages section and current page name is active under published section$")
   public void user_landed_on_view_pages_section_and_current_page_name_is_active_under_published_section() throws Exception {
      namMe.verifyPlubishedPage();
   }
