Feature: Tickets New
  ...
  Scenario: Unable to send with blank spaces in first and last name
    ...
    Then User Verify character limit to notes field %{GD_EMAIL_ADDRESS}
  ...
  Scenario: Verify transfer tickets needs to be sorted in Bulk Transfer when EDP is on
    ...
    Scenario: Verify error message when the user tries to sign in
    And Save "tmf0c53ee4ea@example.com" into EMAIL_ADDRESS 
	  And Save "12345678" into PASSWORD
	  When User provide wrong %{GD_EMAIL_ADDRESS} and %{GD_PASSWORD}
